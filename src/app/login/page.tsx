  ...
  const handleSubmit = async (email: string) => {
    ...
    if (response.status === 'error') {
      if (response.error === 'Signups not allowed for otp') {
        setError('This email has not been invited. Please check your input or contact us to invite you to the wedding.');
      } else {
        setError('Something went wrong. Please try again later or contact us if the problem persists.');
      }
    } else {
      setError(null);
      setEmailSent(true);
